class VoteEventsController < ApplicationController
  ...
  def index
    ...
    if params.has_key?('area') and params.has_key?('num_of_people')
      c = HTTPClient.new
      res = c.get_content 'http://api.gnavi.co.jp/RestSearchAPI/20150630/', {keyid: Rails.application.secrets.gurunavi_access_key, pref:"PREF13", freeword: params["area"], format:"json"}
      res = JSON.parse(res)
